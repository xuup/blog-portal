...
(function() {
  ...
  hm.src = "https://hm.baidu.com/hm.js?a1067ccd3a51a4260412c4a20b100fb5";
...
<script type="text/javascript">
		...
		$.ajax({
			...
			success:function(res){
				...
				for(i=0;i<res.length;i++){
					console.log(i);
					if(i==0){
						beforehtml = "<a href='javascript:void(0)' target='_blank' class='iu'>"+res[i].createTime.substring(8,10)+"</a>";
						className = 'zaiyao';
						className1 = 'mecc';
					}else{
						beforehtml = '';
					...
					"<address class='meccaddress'><time>"+ res[i].createTime +"</time> - <a href=''>"+res[i].categoryName+"</a> 阅读量: "+res[i].readNumber+"</address> </div>"+
	...
	function initPageNumber(){
		...
		var page = new Paging();
		page.init({	target: $('#pageTool'), pagesize: 6, count: total, current:1,callback: function (pagecount, size, count) {
				console.log(arguments)
				//ajax获取页面内容 
				$("#curentPage").val((pagecount-1)*6);
				initArticle();
